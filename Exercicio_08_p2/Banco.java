...
class Banco {
    ...
    public void inserirCliente(Cliente cliente) {
        for (Cliente c : clientes) {
            if (c.getCpf().equals(cliente.getCpf())) {
                throw new JaExisteError("Cliente com CPF " + cliente.getCpf() + " já está cadastrado.");
            }
        }
    ...
    public Conta consultarConta(String numero) {
        ...
        throw new ContaInexistenteError("Nenhuma conta de número " + numero + " foi encontrada.");
    }
    ...
    private int consultarPorIndice(String numero) {
        ...
        throw new ContaInexistenteError("Conta de índice " + indiceProcurado + " não encontrada.");
    }
    ...
    public void efetuarOrdemBancaria(String numeroOrigem, List<String> contasDestino, double valorPorTransferencia) {
        ...
        if (contaOrigem == null) {
            throw new ContaInexistenteError("Conta de origem não encontrada: " + numeroOrigem);
        }

        double valorTotal = contasDestino.size() * valorPorTransferencia;

        if (contaOrigem.getSaldo() < valorTotal) {
            throw new SaldoInsuficienteError("Saldo insuficiente na conta de origem. Saldo disponível: R$" + contaOrigem.getSaldo() + ", necessário: R$" + valorTotal);
        }

        for (String numeroDestino : contasDestino) {
            Conta contaDestino = consultarConta(numeroDestino);
            if (contaDestino == null) {
                throw new ContaInexistenteError("Conta de destino não encontrada: " + numeroDestino);
            }
        }
    ...
    public void atribuirTitularidade(String numeroConta, Cliente cliente) {
        ...
        if (conta.getCliente() != null) {
            throw new ContaJaPossuiClienteError("A conta já possui um cliente associado.");
        }
    ...
    public void transferir(String numeroCredito, String numeroDebito, double valor) {
        ...
        if (contaCredito == null || contaDebito == null) {
            throw new ContaInexistenteError("Uma ou ambas as contas não foram encontradas.");
        }
        if (contaDebito.getSaldo() < valor) {
            throw new SaldoInsuficienteError("Saldo insuficiente na conta de débito.");
        }
    ...
    public void renderJuros(String numeroConta) {
        Conta conta = this.consultarConta(numeroConta);
